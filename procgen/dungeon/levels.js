/* ------------------------------------------------------------------
 *  Dungeon levels — geometry for one or more stacked floors.
 *
 *  buildLevel() carves a single floor (BSP rooms + corridors, or
 *  cellular-automata caves) and layers on room shapes, theme flavor,
 *  the connectivity rescue, a hidden vault, a locked bridge + key, and
 *  room purposes.  buildLevels() stacks 1–3 of them, wires the stairs
 *  between consecutive floors and hands out global room ids.  Geometry
 *  only — encounters / loot / hooks / names are layered on by the
 *  gen-dungeon.js orchestrator.
 *
 *  Tile codes: '#' wall  '.' floor  '+' door  '<' entrance  '>' exit
 *              '~' water (sewer channels / flooded basins)
 *
 *  STREAM-COMPATIBILITY ASYMMETRY (important).  This module was split
 *  out of the old single-floor generator.  To keep every pre-existing
 *  seed's TOP floor byte-identical after the multi-level refactor,
 *  level 0 draws from the ORIGINAL legacy stream keys unchanged:
 *      layout : `${seed}/layout:${size}:${theme}:${density}(+tags)`
 *      shapes : layout.sub('shapes')      flavor : layout.sub('flavor')
 *      locks  : layout.sub('locks')       deco   : `${seed}/deco:${theme}`
 *  Deeper levels use L-suffixed variants derived off the same base
 *  (layout.sub('L'+level) → its .sub('shapes'|'flavor'|'locks');
 *  deco `${seed}/deco:L${level}:${theme}`).  The orchestrator passes
 *  the correct carve stream in as `layoutRng`; buildLevel only decides
 *  the deco key.  This asymmetry is what preserves the identity gate.
 * ------------------------------------------------------------------ */
import { Rng } from '../rng.js';
import { DUNGEON_ROOMS } from '../names.js';

export const SIZES = { s: [40, 28], m: [60, 40], l: [80, 56], den: [34, 24], huge: [100, 68] };
...
export const LAYOUT_TAGS = ['den', 'huge', 'small', 'large', 'loops', 'linear'];
...
/* Danger ladder (mirrors content/dungeon.js DANGER keys) — deeper levels
 * scale one step per floor. */
const DANGER_SCALE = ['safe', 'low', 'medium', 'deadly'];
...
/* ------------------------------------------------------------------
 *  Size / tag / depth resolution
 * ------------------------------------------------------------------ */
export function tagsOf(p) {
    return new Set(String(p.tags || '').toLowerCase().split(/[,\s]+/).map(s => s.trim()).filter(Boolean));
}

/** Resolve the size key (tag overrides; den wins) and its [W, H]. */
function resolveSize(p, tagSet) {
    let sizeKey = p.size;
...
/** Danger word one step deeper per level, clamped to the ladder. */
export function levelDanger(base, level) {
    let i = DANGER_SCALE.indexOf(base);
    if (i < 0) i = 2;                                          // default: medium
    return DANGER_SCALE[Math.min(DANGER_SCALE.length - 1, i + level)];
}

function levelLabel(level, depth) {
    if (depth === 1) return 'Dungeon';
    if (level === 0) return 'Upper Level';
    if (level === depth - 1) return 'The Depths';
    return `Level ${level + 1}`;
}

/* ------------------------------------------------------------------
 *  buildLevel — one floor: carve → shapes → entrance/exit → flavor →
 *  connectivity rescue → secret vault → locked bridge → purposes.
 *  Returns local geometry; ids/stairs/content are layered on later.
 * ------------------------------------------------------------------ */
export function buildLevel(seed, p, level, layoutRng) {
    const tagSet = tagsOf(p);
    const { W, H } = resolveSize(p, tagSet);
    const theme = p.theme;

    const grid = Array.from({ length: H }, () => Array(W).fill('#'));

    const built = (theme === 'caves')
    ...
    const farI = entranceI === u ? v : u;

    grid[rooms[entranceI].cy][rooms[entranceI].cx] = '<';
    if (farI !== entranceI) grid[rooms[farI].cy][rooms[farI].cx] = '>';
    ...
    if (tuning?.pools) {
        ...
        for (const r of rooms) {
            if (r.i === entranceI || r.i === farI) continue;
    ...
    const deco = new Rng(level === 0 ? `${seed}/deco:${theme}` : `${seed}/deco:L${level}:${theme}`);
    let secretI = -1;
    if (p.secrets) {
        const candidates = rooms.filter(r => deg[r.i] === 1 && r.i !== entranceI && r.i !== farI);
    ...
    if (rooms.length >= 6 && lockRng.chance(0.8)) {
        ...
        if (bridges.length) {
            const guarding = bridges.filter(c => !c.near.has(farI));
    ...
    const commonPool = deco.shuffle(DUNGEON_ROOMS.common[theme] || DUNGEON_ROOMS.common.crypt);
    ...
    /* ---- legacy tags (entrance / exit / secret; stairs added later) ---- */
    for (const r of rooms) {
        const tags = [];
        if (r.i === entranceI) tags.push('entrance');
        if (r.i === farI && farI !== entranceI) tags.push('exit');
        if (r.i === secretI) tags.push('secret');
        r.tags = tags;
    }

    return {
        level, label: levelLabel(level, 1), theme,
        W, H, grid, rooms, edges, doors,
        entranceI, farI, secretI, lock, deg,
    };
}

/* ------------------------------------------------------------------
 *  buildLevels — stack 1–3 floors, wire stairs, hand out global ids.
 * ------------------------------------------------------------------ */
export function buildLevels(seed, p, tagSet) {
    const { sizeKey, W, H } = resolveSize(p, tagSet);

    let depth = Math.max(1, Math.min(3, parseInt(p.depth) || 1));
    if (tagSet.has('deep')) depth = 3;
    if (sizeKey === 'den' || sizeKey === 's') depth = Math.min(depth, 2);

    // A crypt/ruins/stronghold delve may bottom out in raw caves.
    const bottomCaves = depth >= 2
        && ['crypt', 'ruins', 'stronghold'].includes(p.theme)
        && new Rng(`${seed}/levels`).chance(0.25);

    // Legacy layout base key — level 0 carves from this DIRECTLY (identity
    // gate); deeper levels use baseLayout.sub('L'+level).
    const layoutTags = LAYOUT_TAGS.filter(t => tagSet.has(t));
    const baseLayout = new Rng(`${seed}/layout:${p.size}:${p.theme}:${p.density}`
        + (layoutTags.length ? ':' + layoutTags.join(',') : ''));

    const floors = [];
    for (let level = 0; level < depth; level++) {
        const isBottom = level === depth - 1;
        const theme = (bottomCaves && isBottom) ? 'caves' : p.theme;
        const lp = theme === p.theme ? p : { ...p, theme };
        const layoutRng = level === 0 ? baseLayout : baseLayout.sub('L' + level);
        const f = buildLevel(seed, lp, level, layoutRng);
        f.label = levelLabel(level, depth);
        floors.push(f);
    }

    const roomsAll = assignGlobalIds(floors);
    const { stairs, stairEdges } = buildStairs(floors);

    return { floors, roomsAll, stairs, stairEdges, W, H, depth, bottomCaves };
}

/* Global room ids, level-ascending; local index (.i) is kept for grid
 * work, .gi/.globalId are the map-wide ids, and .level/.offset let the
 * orchestrator remap per-level edge/lock/secret refs. */
function assignGlobalIds(floors) {
    let gid = 0;
    const roomsAll = [];
...
/* Stairs between consecutive floors: the lower floor's far chamber holds
 * the down-stair ('>'), the upper floor's entrance holds the up-stair
 * ('<') — glyphs already placed by buildLevel; we only mint the paired
 * entities + a stair edge and tag both rooms. */
function buildStairs(floors) {
    const stairs = [], stairEdges = [];
    let n = 0;
    for (let k = 0; k < floors.length - 1; k++) {
        const lo = floors[k], hi = floors[k + 1];
        const down = lo.rooms[lo.farI];
        const up = hi.rooms[hi.entranceI];
        addTag(down, 'stairs'); addTag(up, 'stairs');
        lo.grid[down.cy][down.cx] = '>';
        hi.grid[up.cy][up.cx] = '<';
        stairs.push({ id: 'st' + (++n), kind: 'stair', level: lo.level, to: hi.level, x: down.cx, y: down.cy, room: down.globalId });
        stairs.push({ id: 'st' + (++n), kind: 'stair', level: hi.level, to: lo.level, x: up.cx, y: up.cy, room: up.globalId });
        stairEdges.push({ a: down.globalId, b: up.globalId, kind: 'stair', dir: 'down' });
    }
